...
for (i = 0; i < n; i++) // для каждой строки формируется отдельная строчка 
{
    // создаём после каждой строки n-i отступов от левой стороны консоли, 
    // чем ниже строка, тем меньше отступ
    for (c = 0; c <= (n - i); c++)
    {
        Console.Write(" "); // сначала печатает количество треугольника. 
    }
    for (c = 0; c <= i; c++)
    {
...
        //формула вычисления элементов треугольника
        Console.Write(factorial(i) / (factorial(c) * factorial(i - c)));// печатает по формкле вычисления 
    }
